...
def run_monte_carlo(ticker: str, num_sims: int = 1000, days: int = 252, outfile: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # build paths for plotting: we need the full path (not only ending) for a subset
        ...
        if math.isclose(sigma, 0.0):
            ...
            # reconstruct full paths for plotting for up to plot_sims simulations
            # we already have increments array of shape (days, num_sims)
            # build cumulative log returns with initial 0
        ...
        # Compute percentiles at each time step (if not sigma=0)
